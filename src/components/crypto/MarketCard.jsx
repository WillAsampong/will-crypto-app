  ...
  return (
    <span className="inline-flex tabular-nums text-base leading-[1em] font-normal text-white">
      {paddedNew.map((char, i) => {
        ...
        const animClass = changed && dir ? (dir === "up" ? "flip-up" : "flip-down") : "";
...
const AssetRow = ({ asset }) => {
  ...
  return (
    <div className="flex flex-col w-full py-6 px-4 border-white/8 transition-colors duration-150 rounded-2xl cursor-pointer hover:bg-white/15">
      <div className="flex items-center justify-between w-full gap-4">
        ...
        <div className="flex items-center gap-4 min-w-0 grow">
          ...
          />
          <p className="text-3xl font-semibold text-white m-0 truncate">
            {asset.name}
          </p>
        </div>
...
const SkeletonRow = () => (
  <div className="flex items-center justify-between w-full py-4 border-b border-white/8 animate-pulse">
...
const MarketCard = () => {
  ...
  const { cryptos: tradable } = useCryptos("tradable");
  const { cryptos: gainers }  = useCryptos("gainers");
  const { cryptos: newCoins } = useCryptos("new");
  ...
  // Takes a list of DB coins and enriches each one with live Binance price data.
  // Falls back to the DB price if Binance hasn't loaded yet.
  const mergeWithLive = (dbCoins) => {
    console.log("DB coins:", dbCoins);
    console.log("coinsMap:", coinsMap);

    return dbCoins.map((dbCoin) => {
      // Find the matching Binance entry by symbol (BTC → BTCUSDT)
      const binanceKey = `${dbCoin.symbol}USDT`;
      const live = coinsMap[binanceKey];

      if (!live) return dbCoin; 
  ...
  const getTabData = (tab) => {
    ...
    switch (tab) {
      case "Tradable":
        return mergeWithLive(tradable);
      case "Top gainers":
        return mergeWithLive(gainers);
      case "New on Coinbase":
        return mergeWithLive(newCoins);
      default:
        return [];
    }
  };
  ...
  return (
    <div className="flex flex-col w-full rounded-2xl p-6 min-h-134.5 bg-black">
      {/* Tabs */}
            ...
            return (
              <button
                ...
                className={`
									...
									text-label-1 cursor-pointer transition-all duration-200 font-semibold
									${
                    isActive
